...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ukcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: UKCoind.pid)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    mapMultiArgs["-addnode"].push_back("addnode=104.236.150.155:14354");
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. UKCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. UKCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("UKCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
